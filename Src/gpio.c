...
// signal
LL_GPIO_SetPinMode(       BUT_PORT, SIGNAL_PIN, LL_GPIO_MODE_OUTPUT );
LL_GPIO_SetPinOutputType( BUT_PORT, SIGNAL_PIN, LL_GPIO_OUTPUT_PUSHPULL );
}
...
{
if	( val )
	LL_GPIO_SetOutputPin(   BUT_PORT, SIGNAL_PIN);
else	LL_GPIO_ResetOutputPin( BUT_PORT, SIGNAL_PIN );
}
